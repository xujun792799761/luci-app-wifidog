...
--if fs.access("/usr/bin/wifidog") then
	...
	externalinterface = s:taboption("general",Value,"externalinterface","外网接口","WAN口接口，默认eth1")
	externalinterface.default = "eth1"
	...
	s = m:section(TypedSection, "trustedurllist", "URL白名单列表","")
	s.anonymous = true
	s.addremove = true
	s.template = "cbi/tblsection"

	url = s:option(Value, "url", "url地址")
	url.rmempty  = false
	--url.datatype = "list(urladdr)"
